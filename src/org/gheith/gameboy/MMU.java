...
public class MMU implements Serializable {
    ...
    private int[] vramBank2 = new int[0x1800];
    ...
    public int readByteFromVRAM(int location, boolean isBankOne) {
    	...
    	if (isBankOne) {
    		...
    		return vramBank2[index];
    	}
    }
    
    public void writeByteToVRAM(int location, int data, boolean isBankOne) {
    	int index = location % 0x8000;
    	if (isBankOne) {
    		// Write to bank one
    ...
    public int readByte(int location) {
        ...
        if (location >= 0x8000 && location <= 0x97FF) {
        	return readByteFromVRAM(location, currentVRAMBank == 1);
        }
